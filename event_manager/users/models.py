...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    def get_current_host_visit(self):
        now = timezone.now()
        today_midnight = timezone.make_aware(datetime.combine(date.today(), time()))

        host_visits = self.host_visits.all().filter(in_time__gt=today_midnight, in_time__lte=now).order_by('-in_time')
    ...
    def get_current_visitor_visit(self):
        now = timezone.now()
        today_midnight = timezone.make_aware(datetime.combine(date.today(), time()))
        visitor_visits = self.visitor_visits.all().filter(in_time__gt=today_midnight,
                                                          in_time__lte=now).order_by('-in_time')
